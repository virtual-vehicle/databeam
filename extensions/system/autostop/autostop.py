...
from vif.data_interface.network_messages import Status, IOEvent, StartStop, StartStopCmd, MeasurementStateType

from system.autostop.config import AutostopConfig  # TODO adopt import path and name


@environ.config(prefix='')
class ModuleEnv:
    MODULE_NAME = environ.var(help='Name of this instance', default='Autostop')  # TODO adopt module name
    # TODO define any needed environment variables here


class Autostop(IOModule):  # TODO adopt module name
    def __init__(self, *args, **kwargs):
        ...
        self.autostop_enabled: bool = False
        self.measurement_duration: int = 60
        self.measurement_start: int = 0
    ...
    def issue_stop_message(self):
        message = StartStop(cmd=StartStopCmd.STOP)
        self.module_interface.cm.request(Key(self.module_interface.db_id, 'c', 'cmd_capture'),
                                         data=message.serialize(), timeout=2)

    def _worker_thread_fn(self):
        if not self.autostop_enabled:
            self.logger.debug("Not starting autostop worker.")
            return

        self.logger.debug('worker thread running')
        self.measurement_start = time.time_ns()
        ...
        try:
            while not self._thread_stop_event.is_set():
                ...
                if curr_time - self.measurement_start >= self.measurement_duration * 1e9:
                    self.logger.info("Issuing capture stop command.")
                    stop_thread = threading.Thread(target=self.issue_stop_message)
                    stop_thread.start()
                    break
    ...
    def command_validate_config(self, config: Dict) -> Status:
        if config["measurement_duration"] <= 0:
            return Status(error=True, message="Invalid measurement duration provided.")
    ...
    def command_apply_config(self) -> Status:
        ...
        try:
            ...
            with self._thread_handling_lock:
                ...
                self.autostop_enabled = config["enable_autostop"]
    ...
    def command_state_change(self, command: StartStopCmd, related_state: MeasurementStateType) -> None:
        if related_state == MeasurementStateType.CAPTURING:
            if command == StartStopCmd.START:
                self._start_thread()
            elif command == StartStopCmd.STOP:
                self._stop_thread()
...
if __name__ == '__main__':
    main(Autostop, AutostopConfig, environ.to_config(ModuleEnv).MODULE_NAME)  # TODO adopt module name and config class
