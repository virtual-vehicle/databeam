...
from UdpManager import UdpManager, ErrorCode


@environ.config(prefix='')
class ModuleEnv:
    MODULE_NAME = environ.var(help='Name of this instance', default='UdpSink')


class UdpSink(IOModule):
    ...
    def _worker_thread_fn(self):
        ...
        try:
            while not self._thread_stop_event.is_set():
                ...
                error_code, data = self.udp_manager.receive()
                ...
                if error_code == ErrorCode.OK:
                    ...
                    if not self._thread_stop_event.is_set():
                        self.logger.error("UDP connection broke during data receiving.")
                        break

        except Exception as e:
            self.logger.error(f'Exception in worker: {type(e).__name__}: {e}\n{traceback.format_exc()}')
        self._config_and_restart_udp_manager()
    ...
    def _config_and_restart_udp_manager(self):
        """
        Closes the current udp connection, if one exists, configures everything and directly listens to incoming
        ...
        self.udp_manager.close()
        self.udp_manager.config(config, self._thread_stop_event)
        setup_success = self.udp_manager.setup()
        if setup_success:
            self.logger.info("UDP socket set up for %s:%s",
                             config["udp_address"], config["udp_port"])
        else:
            self.logger.warning("UDP socket could not be set up for %s:%s",
                                config["udp_address"], config["udp_port"])

    def _parse_struct_definition(self, config):
        """
        Parses the struct config definition and passes them to the UdpManager. Incoming struct data is later
        ...
        try:
            self.udp_manager.struct_size = struct.calcsize(struct_format)
            self.udp_manager.struct_format = struct_format
            self.udp_manager.struct_names = struct_names
            self.logger.info(f"STRUCT SIZE = {self.udp_manager.struct_size}")
...
if __name__ == '__main__':
    main(UdpSink, UdpSinkConfig, environ.to_config(ModuleEnv).MODULE_NAME)
